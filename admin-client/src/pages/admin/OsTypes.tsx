...
import { OSType } from "@/types/admin";
...
export default function SoftwareTypes() {
    ...
    const [editingSoftwareType, setEditingSoftwareType] = useState<OSType | null>(null);
    ...
    if (isLoading) {
        return <div>Loading os types...</div>;
    }

    if (error) {
        return <div>Error loading os types: {error.message}</div>;
    }
            ...
            updateSoftwareType.mutate(
                { id: editingSoftwareType.id, data: formData },
                {
                    onSuccess: () => {
                        toast({
                            title: "Success",
                            description: "OS type updated successfully",
                        });
                        resetForm();
            ...
            createSoftwareType.mutate(
                formData,
                {
                    ...
                    onError: (error) => {
                        toast({
                            title: "Error",
                            description: "Failed to create os type",
                            variant: "destructive",
                        });
                    }
                }
    ...
    const handleEdit = (softwareType: OSType) => {
        setFormData({
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-foreground">OS Types</h1>
                    <p className="text-muted-foreground">Manage os categories and their complexity levels</p>
                </div>
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{editingSoftwareType ? "Edit os Type" : "Add New os Type"}</DialogTitle>
                        </DialogHeader>
                ...
                <Card>
                    <CardContent className="pt-6">
                        <div className="text-center py-8">
                            <p className="text-muted-foreground">No os types found matching your search.</p>
                        </div>
                    </CardContent>
                </Card>
            )}
